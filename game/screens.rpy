...
## QTE Variables ###############################################################
# How much have we mashed in total?
default mash_count = 0.0

# How much do we need to mash to complete it?
default max_mash = 10.0

# Are we currently mashing?
default is_mashing = False

# Are we allowed to mash?
default can_mash = False

# What key did we start mashing with?
default start_key = ""

# What key are we currently looking for?
#default wanted_key = ""

# What key is going to change the character's expression?
default trigger_key = ""

# What's the list of wanted keys?
default key_list = []

## Flow Variables ##############################################################
# Where is the current choice menu?
default choice_menu = ""

# Where do we go to start our mashing?
default mash_label = ""

# Where do we go if we fail to mash/run out of time?
default fail_label = ""

# Where do we go if we mash successfully?
default pass_label = ""
...
screen choice(items):
    ...
    key "K_e" action If(is_mashing == False and can_mash == True, true=(SetVariable("mash_count", mash_count+1.0), SetVariable("is_mashing", True), SetVariable("start_key", "e")), false=NullAction())
    key "K_w" action If(is_mashing == False and can_mash == True, true=(SetVariable("mash_count", mash_count+1.0), SetVariable("is_mashing", True), SetVariable("start_key", "w")), false=NullAction())
    key "K_2" action If(is_mashing == False and can_mash == True, true=(SetVariable("mash_count", mash_count+1.0), SetVariable("is_mashing", True), SetVariable("start_key", "2")), false=NullAction())
    #key "K_x" action If(is_mashing == False and can_mash == True, true=(SetVariable("mash_count", mash_count+1.0), SetVariable("is_mashing", True), SetVariable("start_key", "x")), false=NullAction())

    ## while the timer is going, you can hit a button to go back and change your dialogue choice.
    ## a hidden timer will be running in the background, and when that timer ends we go back to the choice label so that more dialogue will go off in the choice screen
        ...
        if mash_count >= max_mash: # if we've reached the counter
            ...
            if start_key in key_list: # if the start key is anywhere in the key list
                ...
                if key_list == [] and pass_label != "": # if there's no more keys to press, continue on
                    ...
                    renpy.call(pass_label)

                else: # if there are more keys to press, return to the prompt and keep going
                    # current_time = total_time
                    renpy.call(mash_label)
...
default current_time = 5.0

# How much time do we start with?
default total_time = 5.0
